...
QList<XMLTestFactory*> HmmerTests::createTestFactories() {
...
OutputCollector::OutputCollector() : ExternalToolListener() {}

void OutputCollector::addNewLogMessage(const QString& message, int messageType) {
    //skip first line of the output hmmsearch calling with parameters
    if (collectedLog.isEmpty() && !message.startsWith("#")) {
        return;
    }
