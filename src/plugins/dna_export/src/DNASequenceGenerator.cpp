...
void DNASequenceGenerator::generateSequence(const QMap<char, qreal> &charFreqs, int length, QByteArray &result, U2OpStatus &os) {
    ...
    QVector<qint64> quotaByChar(256, 0);  // Keeps the counters how many characters can be generated by char code (uchar(c)).
    ...
    for (char ch : qAsConst(chars)) {
        ...
        SAFE_POINT(frequency >= 0 && frequency <= 1, "Invalid frequency: " + QString::number(frequency), );
        qint64 expectedCharCount = length * frequency;
    ...
    // Fill the remaining (rounding error) with the highest frequency char.
    int remaining = length - total;
    if (remaining > 0) {
        char highestFrequencyChar = chars[0];
        for (char ch : qAsConst(chars)) {
            if (charFreqs[ch] > charFreqs[highestFrequencyChar]) {
                highestFrequencyChar = ch;
            }
        }
        quotaByChar[(uchar)highestFrequencyChar] += remaining;
        if (availableChars.isEmpty()) {
            availableChars.append(highestFrequencyChar);
        }
        SAFE_POINT(availableChars.contains(highestFrequencyChar), "No highestFrequencyChar in available chars", );
    }
    SAFE_POINT(!availableChars.isEmpty(), "No available characters to generate sequence.", );
    ...
    for (int idx = 0; idx < length; idx++) {
        ...
        int availableCharIndex = qrand() % availableChars.size();
        char ch = availableChars[availableCharIndex];
        result[idx] = ch;
        quotaByChar[(uchar)ch]--;
        if (quotaByChar[(uchar)ch] == 0) {
            availableChars.removeAt(availableCharIndex);
        }
    }
}
