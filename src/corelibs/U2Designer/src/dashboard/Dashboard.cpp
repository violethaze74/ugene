...
void Dashboard::initLayout() {
    ...
    externalToolsButton = new QToolButton(tabButtonsRow);
    externalToolsButton->setText(tr("External Tools"));
    externalToolsButton->setStyleSheet(tabButtonStyleSheet);
    externalToolsButton->setCursor(Qt::PointingHandCursor);
    externalToolsButton->setCheckable(true);
    externalToolsButton->setVisible(false);
    tabButtonsLayout->addWidget(externalToolsButton);
    ...
    tabButtonGroup->addButton(externalToolsButton, EXTERNAL_TOOLS_TAB_INDEX);
    ...
    auto loadDashboardButton = new QToolButton(tabButtonsRow);
    loadDashboardButton->setIcon(QIcon(":U2Designer/images/load_schema.png"));
//    loadDashboardButton->setText(tr("Open workflow"));
//    loadDashboardButton->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
    loadDashboardButton->setStyleSheet("padding: 4px 6px;");
    loadDashboardButton->setCursor(Qt::PointingHandCursor);
    connect(loadDashboardButton, SIGNAL(clicked()), SLOT(sl_loadSchema()));

    tabButtonsLayout->addWidget(loadDashboardButton);
    tabButtonsLayout->addSpacing(20);
...
void Dashboard::sl_pageReady() {
    ...
    if (isExternalToolsButtonVisibleInHtml(html)) {
        externalToolsButton->setVisible(true);
    }
}
