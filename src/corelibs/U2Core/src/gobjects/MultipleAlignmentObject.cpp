...
GObject * MultipleAlignmentObject::clone(const U2DbiRef &dstDbiRef, U2OpStatus &os, const QVariantMap &hints) const {
    ...
    MultipleAlignmentObject *clonedObj = MultipleSequenceAlignmentImporter::createAlignment(dstDbiRef, dstFolder, msa, os);
...
int MultipleAlignmentObject::deleteGap(U2OpStatus &os, const U2Region &rows, int pos, int maxGaps) {
    ...
    MultipleAlignment ma(getMultipleAlignment()->explicitClone());
...
QMap<qint64, QList<U2MaGap> > MultipleAlignmentObject::getGapModel() const {
...
void MultipleAlignmentObject::sortRowsByList(const QStringList &order) {
    ...
    MultipleSequenceAlignment msa(getMultipleAlignment()->clone());
    msa->sortRowsByList(order);
    CHECK(msa->getRowsIds() != cachedMa->getRowsIds(), );
...
void MultipleAlignmentObject::loadAlignment(U2OpStatus &os) {
    MultipleSequenceAlignmentExporter alExporter;
    cachedMa = alExporter.getAlignment(entityRef.dbiRef, entityRef.entityId, os);
}
