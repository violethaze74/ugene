...
void MsaDbiUtils::insertGaps(const U2EntityRef& msaRef, const QList<qint64>& rowIds, qint64 pos, qint64 count, U2OpStatus& os, bool needToAddGap) {
    // Prepare the connection
    DbiConnection con(msaRef.dbiRef, os);
    CHECK_OP(os, );
    ...
    msaDbi->updateMsaLength(msaRef.entityId, needToAddGap ? msaObj.length + count : msaObj.length, os);
}
