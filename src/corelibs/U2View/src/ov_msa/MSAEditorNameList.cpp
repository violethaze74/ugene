...
MSAEditorNameList::MSAEditorNameList(MaEditorWgt* _ui, QScrollBar* _nhBar)
    : labels(NULL),
      ui(_ui),
      nhBar(_nhBar),
...
int MSAEditorNameList::getSelectedRow() const {
    const MSAEditorSelection& selection = ui->getSequenceArea()->getSelection();
...
void MSAEditorNameList::sl_copyCurrentSequence() {
    ...
    MultipleSequenceAlignmentObject* maObj = editor->getMaObject();
    if (maObj) {
        const MultipleSequenceAlignmentRow row = maObj->getMsaRow(n);
...
void MSAEditorNameList::sl_removeSequence() {
    ...
    MSAEditorSelection oldSelection = ui->getSequenceArea()->getSelection();
...
void MSAEditorNameList::drawSequenceItem(QPainter& p, int s, const QString& , bool selected, const U2Region& yRange, int pos) {
    ...
    const MultipleSequenceAlignment alignment = editor->getMaObject()->getMsa();
    U2OpStatusImpl os;
...
void MSAEditorNameList::drawSelection(QPainter& p) {
    MSAEditorSelection sel = ui->getSequenceArea()->getSelection();
...
void MSAEditorNameList::sl_editSequenceName()
{
    ...
    QString curName =  maObj->getMultipleAlignment()->getMsaRow(n)->getName();
...
bool MSAEditorNameList::isRowInSelection(int seqnum) {
    MSAEditorSelection s = ui->getSequenceArea()->getSelection();
...
qint64 MSAEditorNameList::sequenceIdAtPos(const QPoint &p) {
    ...
    if (curSeq != -1) {
        MultipleSequenceAlignmentObject* maObj = editor->getMaObject();
        result = maObj->getMultipleAlignment()->getMsaRow(curSeq)->getRowId();
    }
    return result;
}
