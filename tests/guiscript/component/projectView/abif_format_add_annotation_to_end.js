...
print("Start testing...")
print("Test :Testin of project view: add annotation to the end of abif file")
...
project_names = new Array(//can't be empty
    "_common_data\\abif\\Q6_78_20090318B.ab1"
);
...
regionSelect = function (modalWid) {
    ...
    QTest.keyClicks(curW, "500", 0, main_delay);

    curW = modalWid.range_selector.end_edit_line;
    QTest.mouseMove(curW, pos, main_delay);
    QTest.mouseDClick(curW, 1, 0, pos, main_delay);
    QTest.keyClicks(curW, "554", 0, main_delay);
...
createNewAnnotation = function (modalWid) {
    ...
    QTest.mouseMove(curW, pos, main_delay);
    QTest.mouseDClick(curW, 1, 0, pos, main_delay);
    QTest.keyClicks(curW, "test_annotation_2", 0, main_delay);

    if (modalWid.CreateAnnotationWidget.groupBox_2.locationEdit.text != "500..554") {
...
pos = new QPoint(100, prVA.height - 20);
        ...
        for (j = 0; treewidget_item.childCount() != j; j++) {
            ...
            if (treewidget_item1.text(0).indexOf("test_group") != -1) {
                for (k = 0; treewidget_item1.childCount() != k; k++) {
                    ...
                    if (treewidget_item2.text(0).indexOf("test_annotation_2") != -1) {
                        readyflag = true;
                    }
                }
            }
