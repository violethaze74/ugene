...
print("Start testing...")
print("Test :Testin of project view: add annotation to the midle of abif file")
...
regionSelect = function (modalWid) {
    ...
    QTest.keyClicks(curW, "100", 0, main_delay);

    curW = modalWid.range_selector.end_edit_line;
    QTest.mouseMove(curW, pos, main_delay);
    QTest.mouseDClick(curW, 1, 0, pos, main_delay);
    QTest.keyClicks(curW, "200", 0, main_delay);
...
createNewAnnotation = function (modalWid) {
    ...
    QTest.mouseMove(curW, pos, main_delay);
    QTest.mouseDClick(curW, 1, 0, pos, main_delay);
    QTest.keyClicks(curW, "test_annotation_3", 0, main_delay);

    if (modalWid.CreateAnnotationWidget.groupBox_2.locationEdit.text != "100..200") {
        ...
        for (j = 0; treewidget_item.childCount() != j; j++) {
            ...
            if (treewidget_item1.text(0).indexOf("test_group") != -1) {
                for (k = 0; treewidget_item1.childCount() != k; k++) {
                    ...
                    if (treewidget_item2.text(0).indexOf("test_annotation_3") != -1) {
                        readyflag = true;
                    }
                }
            }
